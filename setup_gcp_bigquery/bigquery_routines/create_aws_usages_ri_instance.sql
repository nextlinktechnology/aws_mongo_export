BEGIN
-- CREATE OR REPLACE PROCEDURE `${project_id}.${dataset_id}.create_aws_usages_ri_instance`(prefix STRING)

-- create _temp suffix table
EXECUTE IMMEDIATE
format("""
  CREATE OR REPLACE TABLE `${project_id}.${dataset_id}.%saws_usages_ri_instance_temp`
  (
...
EXECUTE IMMEDIATE
format("""
  CREATE TABLE IF NOT EXISTS `${project_id}.${dataset_id}.%saws_usages_ri_instance`
  (
    bill_InvoiceId STRING,
    bill_PayerAccountId STRING,
    fix_adjustment STRING,
