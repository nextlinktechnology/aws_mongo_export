...
data "archive_file" "mongo_export_lambda" {
  ...
  source {
    content  = file("${var.run_path}/setup_aws/mongo_export_lambda/lambda_function.py")
    filename = "lambda_function.py"
  }
  source {
    content  = file("${var.run_path}/setup_aws/mongo_export_lambda/env.py")
    filename = "env.py"
  }
}
...
data "archive_file" "mongo_export_ec2_lambda" {
  ...
  source {
    content  = file("${var.run_path}/setup_aws/mongo_export_ec2_lambda/lambda_function.py")
    filename = "lambda_function.py"
  }
  source {
    content  = file("${var.run_path}/setup_aws/mongo_export_ec2_lambda/env.py")
    filename = "env.py"
  }
}
